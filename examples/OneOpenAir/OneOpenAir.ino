...
#define WIRED_EMIT_INTERVAL 10000                          /** ms (Path A) */

/**
 * Path B: wired client to a host Sensor Hub over USB-C (power + CDC data).
 * Set to 1 to build a wired-transport firmware (skips WiFi/cellular auto-detect
 * and talks to the Sensor Hub instead). Set to 0 for stock WiFi/cellular
 * behavior (Path A one-way emitter still available in that case).
 */
#ifndef AG_WIRED_CLIENT
#define AG_WIRED_CLIENT 1
#endif
...
static WiredEmitter wiredEmitter(Serial, measurements);
static AgSerial *agSerial;
static CellularModule *cellularCard;
static AirgradientClient *agClient;

enum NetworkOption { UseWifi, UseCellular, UseWired };
NetworkOption networkOption;
static StreamByteIO *wireIO = nullptr;
static AgWireLink *wireLink = nullptr;
...
static void wiredEmitHandle(void);
...
AgSchedule wiredEmitSchedule(WIRED_EMIT_INTERVAL, wiredEmitHandle);
...
void setup() {
...
#if AG_WIRED_CLIENT
  // Wired mode always runs the networking task (the Sensor Hub transmit/config
  // schedules run there), so skip the offline-mode button prompt.
  ...
  // Path A: begin wired (USB-CDC) measurement stream. Independent of the
  // network path, so it also runs in offline mode.
  wiredEmitter.begin(fwMode);
  Serial.printf("Wired emitter enabled, interval %d ms\r\n", WIRED_EMIT_INTERVAL);
}
...
void loop() {
  ...
  /* Path A: stream framed measurements out the USB-C port. Disabled in wired
   * client mode, where measurements are posted and ACKed via the transport. */
  if (networkOption != UseWired) {
    wiredEmitSchedule.run();
  }
...
void wiredEmitHandle(void) {
  // Only emit when a USB host is actually present, so a full CDC TX buffer
  // (no reader attached) can never stall the main loop. On the ESP32-C3 the
  // native USB-Serial/JTAG port (HWCDC) exposes host presence via operator bool.
  if (Serial) {
    wiredEmitter.emit(wifiConnector.RSSI());
  }
}
...
void checkForFirmwareUpdate(void) {
  ...
  if (networkOption == UseWired) {
    // No cloud transport in wired mode; firmware is updated via USB-C flashing.
    Serial.println("Wired mode: skipping cloud OTA (flash firmware over USB-C)");
    return;
  }
...
#if AG_WIRED_CLIENT
  // Path B: wired transport to the Sensor Hub over USB-C (power + CDC data).
  Serial.println("Wired client mode: using USB-C link to Sensor Hub");
  wireIO = new StreamByteIO(Serial);
  wireLink = new AgWireLink(*wireIO);
  agClient = new AirgradientSerialClient(*wireLink);
  networkOption = UseWired;
#else
...
void postUsingWired() {
  ...
  // Same payload schema as the WiFi path; delivered framed + ACKed to the hub.
  String payload = measurements.toString(false, fwMode, wifiConnector.RSSI());
  if (agClient->httpPostMeasures(payload.c_str()) == false) {
    Serial.println("Wired post to Sensor Hub failed, retry next schedule");
  }
}
...
void sendDataToServer(void) {
  ...
  } else if (networkOption == UseWired) {
    postUsingWired();
  }
}
...
void networkingTask(void *args) {
  ...
  while (1) {
    ...
    if (networkOption == UseWifi) {
      ...
      if (agClient->isClientReady() == false) {
        Serial.println("Wired link not ready, re-handshaking with Sensor Hub...");
        if (agClient->ensureClientConnection(false) == false) {
          delay(5000); // Sensor Hub not responding yet, retry
          continue;
        }
      }
    }
